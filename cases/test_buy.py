...
handel_excel = HandleExcel(AIDONG_DATA_PATH_FILE,sheet='buy')
...
class TestBuy(unittest.TestCase):
    ...
    def test_buy(self, val):
        ...
        actual_result = token_request(method=method, url=new_url,data=datas)
        if case_id == 1:
        ...
        # print(type(expect))
        if case_id == 2:
            code_id = actual_result.json()["data"]["product"]["spec"]["item"][1]["code"]
            print(code_id)
