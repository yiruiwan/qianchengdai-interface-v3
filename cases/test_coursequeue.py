...
handel_excel = HandleExcel(AIDONG_DATA_PATH_FILE,sheet='coursequeue')
...
class TestCourseL(unittest.TestCase):
    ...
    @data(*case)
    def test_course_queue(self,val):
        do_log.info("\nRunning Test Method: {}".format(inspect.stack()[0][3]))
        case_id = val['case_id']
        response_url = val['url']
        datas = val['data']
        method = val['method']
        new_url = HandleContext().course_queue(response_url)
